"""OSM Overpass source — bank per city bbox."""
...
MAX_PER_CITY = int(os.getenv("OVERPASS_BANK_MAX", "200"))
...
[out:json][timeout:90];
(
  node["amenity"="bank"]["name"]({bb});
  way["amenity"="bank"]["name"]({bb});
);
out center tags;
"""
...
def _map_elements(data: dict[str, Any], city: str) -> list[dict[str, Any]]:
    ...
    if len(results) > MAX_PER_CITY:
        log.info("bank cap %s → %s for city=%s", len(results), MAX_PER_CITY, city)
        results = results[:MAX_PER_CITY]
    return results


def fetch_banks(city: str | None = None) -> list[dict[str, Any]]:
    city_code = city or os.getenv("CRAWL_CITY") or "hanoi"
    query = _build_query(get_bbox(city_code))
    try:
        data = fetch_overpass_json(query)
    except RuntimeError:
        seed = load_city_seed("bank", city_code, "bank")
        if seed:
            return seed
        raise

    results = _map_elements(data, city_code)
    log.info("Overpass returned %s bank for %s", len(results), city_code)
    if not results:
        seed = load_city_seed("bank", city_code, "bank")
        if seed:
            return seed
