"""OSM Overpass source — school per city bbox."""
...
MAX_PER_CITY = int(os.getenv("OVERPASS_SCHOOL_MAX", "400"))
...
[out:json][timeout:90];
(
  node["amenity"="school"]["name"]({bb});
  way["amenity"="school"]["name"]({bb});
);
out center tags;
"""
...
def _map_elements(data: dict[str, Any], city: str) -> list[dict[str, Any]]:
    ...
    for el in data.get("elements", []):
        rec = map_osm_element(
            ...
            city=city,
            loc_type="school",
            default_name_prefix="Trường",
        )
        if not rec:
            continue

        name = str(rec["name"])
        if len(name) < 4 or name.upper() in {"A", "B", "C"}:
            continue
    ...
    if len(results) > MAX_PER_CITY:
        log.info("school cap %s → %s for city=%s", len(results), MAX_PER_CITY, city)
        results = results[:MAX_PER_CITY]
    return results


def fetch_schools(city: str | None = None) -> list[dict[str, Any]]:
    city_code = city or os.getenv("CRAWL_CITY") or "hanoi"
    query = _build_query(get_bbox(city_code))
    try:
        data = fetch_overpass_json(query)
    except RuntimeError:
        seed = load_city_seed("school", city_code, "school")
        if seed:
            return seed
        raise

    results = _map_elements(data, city_code)
    log.info("Overpass returned %s school for %s", len(results), city_code)
    if not results:
        seed = load_city_seed("school", city_code, "school")
        if seed:
            return seed
